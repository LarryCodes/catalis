...
@if(!$selectedPartnerId)
                            ...
                            <td>
                                <div class="actions-dropdown-wrapper" x-data="{ open: false }">
                                    ...
                                    <div class="actions-dropdown-panel" x-show="open" x-cloak>
                                        @can('edit-sites')
                                            <button class="action-item" wire:click="openEditForm({{ $site->id }})" @click="open = false">
                                        ...
                                        @can('delete-sites')
                                            <button class="action-item deactivate" 
                                                    wire:click="deactivateSite({{ $site->id }})"
                                                    wire:confirm="Are you sure you want to deactivate this site?"
                                                    @click="open = false">
        ...
        @else
            <div class="empty-state">
                <h3>No sites yet</h3>
                <p>Use the "Add Site" button to create the first site for this partner.</p>
            </div>
        @endif
    </div>
    ...
    <div class="modal-overlay" wire:click.self="closeForm">
        <div class="modal-content" wire:click.stop>
            ...
            <div class="modal-body">
                <livewire:site-form :site="$editingSite" :partner-id="$selectedPartnerId" :key="'site-form-'.($editingSite ? $editingSite->id : 'new-'.$selectedPartnerId)" />
            </div>
        </div>
    </div>
    @endif
