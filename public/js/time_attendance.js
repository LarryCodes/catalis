document.addEventListener('DOMContentLoaded', () => {
  ...
  const actionBar = document.getElementById('ta-actions');
  ...
  if (deactivateBtn) {
    deactivateBtn.addEventListener('click', () => {
      const selected = getRowCheckboxes().filter(cb => cb.checked);
      if (!selected.length) return alert('No records selected.');
      alert(`${selected.length} record(s) marked for deactivation (placeholder).`);
    });
  }
  ...
  function performLiveSearch() {
    const tableBody = document.querySelector('.ta-table tbody');
    if (!tableBody) return;
  ...
  function updateNoResultsRow(tbody, term, visibleCount) {
    ...
    if (term && visibleCount === 0) {
      ...
      noDataRow.innerHTML = '<td colspan="12" style="text-align: center;">No data available.</td>';
  ...
  function ensureNoDataRow() {
    ...
    if (!hasData) {
      const message = searchTerm ? 'No results found.' : 'No data available.';
